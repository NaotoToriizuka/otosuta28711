...
RSpec.describe '教室情報編集', type: :system do
  ...
  context '教室情報編集ができないとき' do
    ...
    it 'ログインしているがユーザーが登録していない教室は編集できない' do
      ...
      # 編集ページがないことを確認する
      expect(page).to have_no_content('教室情報の編集')
    end
...
RSpec.describe '教室情報削除', type: :system do
  ...
  context '教室情報が削除できるとき' do
    it 'ログインしたユーザーは自分が登録した教室情報の削除ができる' do
      ...
      # 教室一覧ページに遷移する
      visit rooms_path
      # 登録した名前が削除され表示されていないことを確認する
      expect(page).to have_no_content('Otosuta教室')
    end
  end
end
...
RSpec.describe '教室情報削除', type: :system do
    before do
      ...
      context '教室情報が削除できないとき' do
    it 'ログインしているがユーザーが登録していない教室は削除できない' do
      # 教室1を登録したユーザーでログインする
      sign_in(@music_genre1.music)
      # 教室一覧ページへ遷移する
      visit rooms_path
      ...
      visit room_path(@music_genre2.music)
      # 削除ボタンがないことを確認する
      expect(page).to have_no_content('教室情報の削除')
    end
    it 'ログインしていないユーザーは教室情報を削除できない' do
      # 教室一覧ページへ遷移する
      visit rooms_path
      # 登録した名前から詳細ページに遷移する
      visit room_path(@music_genre1.music)
